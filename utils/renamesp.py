...
for f in files:
    ...
    with fileinput.FileInput(f, inplace=True) as fi:
        for line in fi:
            for t in targets:
                line = line.replace(f'using namespace {t}', 'using namespace clmdep_' + t)
                line = line.replace(f'namespace {t}', 'namespace clmdep_' + t)
    ...
    with fileinput.FileInput(f, inplace=True) as fi:
        for line in fi:
            for a, b in usages.items():
                line = line.replace(f'using namespace {a}', 'using namespace ' + b)
